...
class plgContentfreeclupGallery extends JPlugin {

  // Freeclup reference parameters
	var $plg_name								= "freeclupGallery";
	var $plg_copyrights_start		= "\n\n<!-- Freeclup \"Freeclup Image Gallery\" Plugin (v2.2) starts here -->\n";
	var $plg_copyrights_end			= "\n<!-- Freeclup \"Freeclup Image Gallery\" Plugin (v2.2) ends here -->\n\n";
	var $plg_tag								= "pic";  //xman1
	...
	// $this->params->get('relative');
	$view = JRequest::getString('view', '');  //<-- control show in bloag
	//echo $view ;
	//echo  $row->state ;featured
	//die();
    if ( $view == 'article' && $row->state == 1)
    {
		...
		//$mainframe	= &JFactory::getApplication();  //orginal xman
		...
		if(!extension_loaded('gd') && !function_exists('gd_info')) {
			JError::raiseNotice('', JText::_('FREECLUPGALLERY_NGD'));
		}
		if(!is_writable($sitePath.DS.'cache')){
			JError::raiseNotice('', JText::_('FREECLUPGALLERY_CFU'));
		}
		...
		$on						= $this->params->get('jqoff', 1);
		

		//$off						= $this->params->get('jqoff', 0);
		// Advanced
		...
		$cacheFolder						= 'cache/freeclupGallery';

		// Other assignments
		$transparent						= $siteUrl."/plugins/content/".$this->plg_name."/".$this->plg_name."/includes/images/transparent.gif";

		// Includes
		//echo "dirname(__FILE__).DS.$this->plg_name.DS.'includes'.DS.'helper.php'";
		//die();
		...
		if(preg_match_all($regex, $row->text, $matches, PREG_PATTERN_ORDER) > 0) {
			...
			foreach ($matches[0] as $key => $match) {
				...
				if(!$gallery){
					JError::raiseNotice('',JText::_('FREECLUPGALLERY_PRG'));
					return;
				}
				...
				// Output
				$plg_html = $getTemplate; //print output
		...
		//---------------------------------------remove tag in article featured -------------------------------------------
	if ( $view == 'featured' )
	{
		// expression to search for
		$regex = "#{".$this->plg_tag."}(.*?){/".$this->plg_tag."}#s";
